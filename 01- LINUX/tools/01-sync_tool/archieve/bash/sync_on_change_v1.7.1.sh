...
# -----------------------------------------------------------------------------
# Script: sync_on_change_v1.7.1.sh
# Version: 1.7.1
#
# Purpose:
...
"
  # Write example config to CWD (where the user runs the script from)
  local EXAMPLE_CONF_CWD="$PWD/.sync_on_change.conf.example"
  if [ ! -f "$EXAMPLE_CONF_CWD" ]; then
    printf '%s' "$EXAMPLE_CONTENT" > "$EXAMPLE_CONF_CWD"
    debug "Example config written to: $EXAMPLE_CONF_CWD"
  fi

  # If root, also create system-wide reference in /etc/sync_on_change/
  if [ "$(id -u)" -eq 0 ]; then
