...
# -----------------------------------------------------------------------------
# Script: sync_on_change_v1.7.2.sh
# Version: 1.7.2
#
# Purpose:
...
"
  ...
  if [ -n "$config_loaded" ]; then
    # Config was found and loaded — offer an example only if one doesn't exist yet
    if [ ! -f "$EXAMPLE_CWD" ]; then
      echo
      info "Current settings are loaded from: $config_loaded"
      read -t 15 -p $'[?] Would you like a reference example config saved alongside it? (y/N): ' _answer
      echo
      if [[ "$_answer" =~ ^[Yy]$ ]]; then
        printf '%s' "$EXAMPLE_CONTENT" > "$EXAMPLE_CWD"
        info "Example config saved to: $EXAMPLE_CWD"
      fi
    fi
  else
