...
# -----------------------------------------------------------------------------
# Script: sync_on_change_v1.7.3.sh
# Version: 1.7.3
#
# Purpose:
...
load_config() {
  ...
  if [ -n "$config_loaded" ]; then
    ...
    info "Tip: create a config file at '$PWD/.sync_on_change.conf' to customise settings."
  fi
}
...
ensure_system_config_dir_and_example() {
  ...
  EXAMPLE_CONTENT="# sync_on_change configuration file
...
"
  ...
  local CONF_CWD="$PWD/.sync_on_change.conf"
  local EXAMPLE_CWD="$PWD/.sync_on_change.conf.example"
  local _answer

  if [ -n "$config_loaded" ]; then
    # Config found — silently note if example already exists, otherwise just inform (no action needed)
    if [ ! -f "$EXAMPLE_CWD" ]; then
      info "Configuration is active from: $config_loaded — no configurations example file needed."
    fi
    # Example already exists: fully silent, no clutter
  else
    # No config found — guide the user to create one
    echo
    warn "No config file found at '$CONF_CWD' (or any fallback location)."
    read -t 15 -p $'[?] Would you like to generate an example config file to get started? (y/N): ' _answer
    echo
    if [[ "$_answer" =~ ^[Yy]$ ]]; then
      printf '%s' "$EXAMPLE_CONTENT" > "$EXAMPLE_CWD"
      info "Example config created at: $EXAMPLE_CWD"
      info "Rename it to '.sync_on_change.conf' and fill in your remote details."
    else
      info "No example config created. Continuing with built-in defaults."
    fi
  fi
  ...
  if [ "$(id -u)" -eq 0 ]; then
    ...
    [ -f "$EXAMPLE_ETC" ] || printf '%s' "$EXAMPLE_CONTENT" > "$EXAMPLE_ETC"
  fi
}
...
info "Watching $target_type : $target"
info "Remote          : ${remote_user}@${remote_host}:${remote_path}"
