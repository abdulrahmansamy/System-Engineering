...
# Usage: fetch_single_secret_generate_lines.sh <username> <secret_id> [--pg-port 5432] [--pgbouncer-port 6432] [--pg-hba-cidr 192.168.24.0/0]
# Prints:
...
PG_PORT=5432
PGBOUNCER_PORT=6432
PG_HBA_CIDR="192.168.24.0/0"  # default as requested

# Parse args
if [[ $# -lt 2 ]]; then
  echo "Usage: $0 <username> <secret_id> [--pg-port 5432] [--pgbouncer-port 6432] [--pg-hba-cidr 192.168.24.0/0]" >&2
  exit 1
fi
...
echo "host    all             ${USERNAME}       ${PG_HBA_CIDR}               md5"
