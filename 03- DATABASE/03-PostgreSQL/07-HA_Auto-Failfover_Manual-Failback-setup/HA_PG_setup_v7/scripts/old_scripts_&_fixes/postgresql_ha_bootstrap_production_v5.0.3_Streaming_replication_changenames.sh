...
install_packages() {
  ...
  retry 5 1 apt install -y wget ca-certificates gnupg lsb-release curl gpg
...
EOF
  ...
  success "PgBouncer userlist file created with MD5 password hashes"
}
...
EOF
  ...
  sudo -u postgres psql -c "SELECT rolname, rolreplication, rolcanlogin FROM pg_roles WHERE rolname IN ('repuser', 'monitor_user', 'app_user', 'postgres') ORDER BY rolname;" || true
...
EOF
  ...
  info "Timeout: 60 seconds (1 minute)"

  local standby_wait_timeout=60
...
SYNC_EOF
      ...
      if [[ "$sync_state" == "sync" ]]; then
        ...
        warn "⚠️ Standby sync state: $sync_state (expected: sync)"
      fi
...
sync_database_passwords() {
  ...
  if [[ "$is_standby" == "t" ]]; then
    ...
    info "Converting all user passwords to MD5 for PgBouncer..."
    sudo -u postgres psql <<EOF || warn "Some password conversions may have failed"
...
-- Ensure app_user and monitor_user have proper database access
GRANT CONNECT ON DATABASE postgres TO app_user;
GRANT CONNECT ON DATABASE postgres TO monitor_user;
GRANT USAGE ON SCHEMA public TO app_user;
...
setup_pg_extensions() {
  info "Setting up PostgreSQL extensions..."
  ...
  if [[ "$is_standby" == "t" ]]; then
    ...
    info "Skipping extension creation on read-only standby node"
    return 0
  fi
  
  # Create extensions on primary node
  info "Creating PostgreSQL extensions on primary node..."
  
  sudo -u postgres psql -d postgres <<EOF || warn "Some extensions may have failed to create"
...
EOF
  ...
  info "Verifying installed extensions:"
  sudo -u postgres psql -d postgres -c "SELECT extname, extversion FROM pg_extension WHERE extname IN ('pg_stat_statements', 'pg_buffercache', 'uuid-ossp') ORDER BY extname;" || true
  
  success "PostgreSQL extensions (pg_stat_statements, pg_buffercache, uuid-ossp) created successfully"
}
