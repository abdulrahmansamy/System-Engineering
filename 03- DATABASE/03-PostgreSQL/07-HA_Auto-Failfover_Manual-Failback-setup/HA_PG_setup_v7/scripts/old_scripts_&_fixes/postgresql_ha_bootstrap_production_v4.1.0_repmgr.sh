...
#
# Version: 4.1.0 - Enhanced Auto-Failover: Optimized repmgr settings, improved service startup, better error handling
...
generate_repmgr_conf() {
  info "Generating repmgr configuration with optimized auto-failover settings..."
  local node_id conninfo_host node_name
  
  # Ensure we have the latest PostgreSQL paths
  detect_pg_paths
  
  # Fix node naming to match actual roles correctly
  case "$ROLE" in
    primary) 
      node_id=1
      node_name="${ROLE}-node"
...
# OPTIMIZED AUTO-FAILOVER SETTINGS
...
primary_notification_timeout=8
...
# Commands for failover operations
promote_command='repmgr standby promote -f ${REPMGR_CONF_FILE} --log-to-file'
follow_command='repmgr standby follow -f ${REPMGR_CONF_FILE} --log-to-file --upstream-node-id=%n'
...
# Child node connection parameters
child_nodes_check_interval=5
child_nodes_connected_min_count=0
child_nodes_disconnect_timeout=15
...
EOF
  ...
  # Configure file descriptor limits to prevent "Too many open files" errors
  info "Configuring file descriptor limits for PostgreSQL..."
...
init_primary() {
  ...
  # Create users and database with enhanced error handling
  info "Creating database users with MD5 authentication for repmgr compatibility..."
  sudo -u postgres psql <<EOF
-- Set password encryption to md5 for PgBouncer/repmgr compatibility
SET password_encryption = 'md5';

-- Create repmgr user with enhanced permissions
DO \$\$
BEGIN
    -- Drop and recreate to ensure clean state
    DROP USER IF EXISTS ${REPMGR_USER};
    CREATE USER ${REPMGR_USER} WITH SUPERUSER REPLICATION LOGIN;
...
-- Grant additional permissions to repmgr user
GRANT ALL ON DATABASE ${REPMGR_DB} TO ${REPMGR_USER};
GRANT pg_monitor TO ${REPMGR_USER};

-- Reset password encryption
RESET password_encryption;
EOF
  
  success "Database users created with proper permissions"
...
#!/bin/bash
NODE_ID="$1"; EVENT="$2"; SUCCESS="$3"; TIME="$4"; DETAILS="$5"; PRIMARY_NODE="$6"; RECOVERY_NODE="$7"
...
case "$EVENT" in
  standby_promote) logger -t repmgr "$([ "$SUCCESS" == "1" ] && echo "SUCCESS" || echo "FAILED"): Node $NODE_ID promotion" ;;
  repmgrd_failover_promote) logger -t repmgr "FAILOVER: Automatic promotion initiated on node $NODE_ID" ;;
esac
EOF
...
EOF
  ...
  # Create the WORKING repmgrd service configuration (based on successful deployment)
...
ExecStart=/usr/lib/postgresql/${PG_VERSION}/bin/repmgrd -f ${REPMGR_CONF_FILE} --daemonize
...
start_services() {
  ...
  # Start repmgrd (only for primary/standby) with enhanced startup and validation
  if [[ "$ROLE" != "witness" ]]; then
