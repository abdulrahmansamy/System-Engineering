...
EOL
    ...
    # Step 5: Use proven pg_basebackup method with enhanced options
    info "Step 5: Using proven pg_basebackup method with WAL streaming..."
    info "This may take several minutes - copying all database files..."
    
    local cloning_success=false
    
    # Enhanced pg_basebackup with all proven options
    if timeout 600 ssh -A $SSH_OPTIONS "$SSH_USER@$failed_primary_ssh_host" "
        sudo -u postgres env PGPASSWORD='$PG_SUPER_PASS' pg_basebackup \\
            ...
            --write-recovery-conf
    " 2>&1; then
        success "✅ pg_basebackup with proven configuration completed successfully"
        ...
        if ssh $SSH_OPTIONS "$SSH_USER@$failed_primary_ssh_host" "sudo -u postgres env PGPASSFILE=/var/lib/postgresql/.pgpass repmgr -h $new_primary_ip -U repmgr -d repmgr -f /etc/repmgr/repmgr.conf standby clone --force" 2>/dev/null; then
            ...
            # Method 3: Manual initialization with proven configuration
            ssh $SSH_OPTIONS "$SSH_USER@$failed_primary_ssh_host" "
...
primary_conninfo = 'host=$new_primary_ip port=5432 user=repmgr application_name=standby'
hot_standby = on
wal_level = replica
max_wal_senders = 10
...
fix_repmgr_upstream() {
    ...
    primary_role=$(timeout 5 env PGPASSWORD="$PG_SUPER_PASS" psql -h "$PRIMARY_IP" -p "$DB_PORT" -U "$USERNAME" -d "$DATABASE" -Atqc "SELECT CASE WHEN pg_is_in_recovery() THEN 'STANDBY' ELSE 'PRIMARY' END;" 2>/dev/null || echo "UNKNOWN")
    standby_role=$(timeout 5 env PGPASSWORD="$PG_SUPER_PASS" psql -h "$STANDBY_IP" -p "$DB_PORT" -U "$USERNAME" -d "$DATABASE" -Atqc "SELECT CASE WHEN pg_is_in_recovery() THEN 'STANDBY' ELSE 'PRIMARY' END;" 2>/dev/null || echo "UNKNOWN")
...
ultimate_repmgr_fix() {
    ...
    # Step 1: Show current replication status
    info "🔍 Step 1: Current replication status on primary..."
    local current_replication
    current_replication=$(ssh $SSH_OPTIONS "$SSH_USER@$PRIMARY_SSH_HOST" "sudo -u postgres psql -c \"
        SELECT 
    ...
    ssh $SSH_OPTIONS "$SSH_USER@$STANDBY_SSH_HOST" "
        ...
        echo \"primary_conninfo = 'host=$PRIMARY_IP port=5432 user=repmgr application_name=standby'\" | sudo -u postgres tee -a /var/lib/postgresql/17/main/postgresql.conf
    ...
    while [[ $reconnection_wait -lt $max_reconnection_wait ]]; do
        local repl_status
        repl_status=$(ssh $SSH_OPTIONS "$SSH_USER@$PRIMARY_SSH_HOST" "sudo -u postgres psql -Atqc \"
            SELECT count(*) 
            FROM pg_stat_replication 
            WHERE client_addr = '$STANDBY_IP' AND state = 'streaming';
        \"" 2>/dev/null || echo "0")
    ...
    final_replication=$(ssh $SSH_OPTIONS "$SSH_USER@$PRIMARY_SSH_HOST" "sudo -u postgres psql -c \"
        SELECT 
    ...
    final_cluster_status=$(ssh $SSH_OPTIONS "$SSH_USER@$PRIMARY_SSH_HOST" "sudo -u postgres repmgr -f /etc/repmgr/repmgr.conf cluster show" 2>/dev/null)
...
comprehensive_health_check() {
    ...
    if [[ "$primary_ip_role" == "PRIMARY" && "$standby_ip_role" == "STANDBY" ]]; then
        ...
        info "  💡 Use option 12 (Fail Back) to restore original configuration"
    else
    ...
    while true; do
        ...
        case $choice in
            ...
            16) 
                ...
                break
                ;;
            *) warn "Invalid choice. Please select 1-16." ;;
        esac
