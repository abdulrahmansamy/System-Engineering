...
# Health endpoint configuration - Updated for production deployment
...
[Service]
Type=simple
# Proactively kill any conflicting processes on port 8001
ExecStartPre=/bin/bash -c "pkill -f 'final-pg-health' || true; pkill -f ':8001' || true; lsof -ti:8001 2>/dev/null | xargs -r kill -9 || true; sleep 1"
...
[Service]
Type=simple
# Proactively kill any conflicting processes on port 8002
ExecStartPre=/bin/bash -c "pkill -f 'final-pgbouncer-health' || true; pkill -f ':8002' || true; lsof -ti:8002 2>/dev/null | xargs -r kill -9 || true; sleep 1"
...
start_services() {
  ...
  if [[ "$ROLE" != "witness" ]]; then
    ...
    if systemctl start final-pgbouncer-health.service; then
      ...
      warn "Failed to start PgBouncer health service - checking logs"
      journalctl -u final-pgbouncer-health.service --lines=10 --no-pager || true
    fi
  fi
...
def check_postgresql_health():
    ...
    try:
        ...
        role_check = subprocess.run(
            ['sudo', '-u', 'postgres', 'psql', '-tAc', 'SELECT pg_is_in_recovery();', 'postgres'],
