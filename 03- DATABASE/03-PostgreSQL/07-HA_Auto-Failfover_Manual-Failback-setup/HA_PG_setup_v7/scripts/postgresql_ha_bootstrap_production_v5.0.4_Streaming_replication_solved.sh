...
# - LVM-based data volumes configuration with mounting and formatting.
# - PostgreSQL 17 + Streaming Replication HA cluster with automatic failover
...
# - pg_stat_statements pg_buffercache uuid-ossp extensions pre-installed
...
#
# Version: 5.0.4 - Production-Ready: Fixed all warnings, improved timing, enhanced error handling
#
# Changelog v5.0.4:
# - Reduced standby wait timeout from 120s to 60s (faster bootstrap)
# - Fixed PgBouncer monitor_user MD5 authentication with proper conversion
# - Added retry logic for WAL receiver status checks on standby
# - Enhanced extension verification with retry logic (handles replication timing)
# - Improved replication connection checks with proper retries
# - Changed all WARN messages for expected behaviors to INFO
# - Added comprehensive troubleshooting for health endpoint failures
...
setup_lvm_storage() {
  info "Setting up LVM storage for PostgreSQL data..."
  
  # Configuration
  local disk="${POSTGRES_DISK:-/dev/sdb}"
  local vg_name="${POSTGRES_VG:-pg_vg}"
  local lv_name="${POSTGRES_LV:-pg_lv}"
  local mount_point="${POSTGRES_MOUNT:-/var/lib/postgresql}"
  
  # Check if disk exists
  if [[ ! -b "$disk" ]]; then
    warn "Disk $disk not found - skipping LVM setup"
    return 0
  fi
  
  # Check if already mounted
  if mountpoint -q "$mount_point"; then
    info "Storage already mounted at $mount_point - skipping LVM setup"
    return 0
  fi
  
  # Check if volume group already exists
  if vgs "$vg_name" >&/dev/null; then
    info "Volume group $vg_name already exists"
    
    # Check if logical volume exists
    if lvs "${vg_name}/${lv_name}" >&/dev/null; then
      info "Logical volume $lv_name already exists"
      ...
      if ! mountpoint -q "$mount_point"; then
        info "Attempting to mount existing logical volume..."
        
        # Check if filesystem exists on the logical volume
        local fs_type
        fs_type=$(blkid -s TYPE -o value "/dev/${vg_name}/${lv_name}" 2>/dev/null || echo "none")
        
        if [[ "$fs_type" == "none" || -z "$fs_type" ]]; then
          warn "No filesystem detected on logical volume, creating XFS filesystem..."
          if ! mkfs.xfs -f "/dev/${vg_name}/${lv_name}"; then
            error "Failed to create filesystem on existing logical volume"
            return 1
          fi
          success "XFS filesystem created on existing logical volume"
        else
          info "Detected existing filesystem: $fs_type"
        fi
        
        # Try to mount with detected or newly created filesystem
        if mount "/dev/${vg_name}/${lv_name}" "$mount_point" 2>/dev/null; then
          success "Mounted existing logical volume"
        else
          warn "Mount failed, checking filesystem integrity..."
          
          # Try to repair XFS filesystem if it's XFS
          if [[ "$fs_type" == "xfs" ]]; then
            info "Attempting XFS filesystem repair..."
            xfs_repair "/dev/${vg_name}/${lv_name}" 2>/dev/null || {
              warn "XFS repair failed or completed with warnings"
            }
            
            # Retry mount after repair
            if mount "/dev/${vg_name}/${lv_name}" "$mount_point" 2>/dev/null; then
              success "Mounted logical volume after repair"
            else
              error "Failed to mount logical volume even after repair attempt"
              info "Recreating filesystem as last resort..."
              mkfs.xfs -f "/dev/${vg_name}/${lv_name}" || {
                error "Failed to recreate filesystem"
                return 1
              }
              mount "/dev/${vg_name}/${lv_name}" "$mount_point" || {
                error "Failed to mount even after recreating filesystem"
                return 1
              }
              success "Mounted with freshly created filesystem"
            fi
          else
            error "Failed to mount existing logical volume with filesystem type: $fs_type"
            return 1
          fi
        fi
      else
        info "Logical volume already mounted at $mount_point"
      fi
      
      # Ensure proper ownership
      chown -R postgres:postgres "$mount_point" 2>/dev/null || true
      
      return 0
    fi
  fi
  
  info "Configuring new LVM storage on $disk..."
  ...
  if ! command -v pvcreate >&/dev/null; then
    ...
    retry 5 2 apt update -y || {
      warn "Failed to update package lists, continuing anyway"
    }
    
    # Install LVM tools with retry logic
    if ! retry 3 2 apt install -y lvm2 xfsprogs; then
      error "Failed to install LVM tools after retries"
      return 1
    fi
    
    success "LVM tools installed successfully"
  fi
  
  # Wipe existing signatures
  info "Wiping existing signatures on $disk"
  wipefs -a "$disk" 2>/dev/null || {
    warn "Failed to wipe signatures, continuing anyway"
  }
  
  # Create physical volume
  info "Creating physical volume on $disk"
  ...
  # Create logical volume (use 100% of free space)
  info "Creating logical volume: $lv_name"
  if ! lvcreate --yes -n "$lv_name" -l 100%FREE "$vg_name"; then
    error "Failed to create logical volume"
    return 1
  fi
  
  # Format with XFS
  info "Formatting logical volume with XFS filesystem"
  if ! mkfs.xfs "/dev/${vg_name}/${lv_name}"; then
    error "Failed to format logical volume"
    return 1
  fi
  ...
  info "Mounting logical volume to $mount_point"
  if ! mount "/dev/${vg_name}/${lv_name}" "$mount_point"; then
    error "Failed to mount logical volume"
    return 1
  fi
  
  # Set ownership for postgres user (will be set later after PostgreSQL is installed)
  info "Setting initial permissions on mount point"
  chmod 755 "$mount_point"
  
  # Mark that we need to set ownership later
  export LVM_MOUNT_NEEDS_OWNERSHIP="true"
  ...
  if [[ -z "$uuid" ]]; then
    warn "Could not determine UUID, using device path in fstab"
    uuid="/dev/${vg_name}/${lv_name}"
  else
    uuid="UUID=$uuid"
  fi
  ...
  if ! grep -q "$mount_point" /etc/fstab; then
    info "Adding entry to /etc/fstab for persistent mounting"
    echo "$uuid  $mount_point  xfs  defaults,noatime  0 2" >> /etc/fstab
    success "Added fstab entry for automatic mounting at boot"
  else
    info "Mount point already exists in /etc/fstab"
  fi
  
  # Verify setup
  info "Verifying LVM storage setup..."
  info "Block devices:"
  lsblk "$disk" 2>/dev/null || true
  info "Filesystem usage:"
  df -h "$mount_point" 2>/dev/null || true
  info "LVM configuration:"
  pvdisplay "$disk" 2>/dev/null | grep -E "PV Name|VG Name|PV Size" || true
  lvdisplay "/dev/${vg_name}/${lv_name}" 2>/dev/null | grep -E "LV Path|LV Name|LV Size" || true
  
  success "LVM storage configured successfully: $disk -> $mount_point ($(df -h "$mount_point" | tail -1 | awk '{print $2}'))"
}
...
set_hostname() {
  info "Configuring hostname based on node role..."
  
  local new_hostname
  local current_hostname=$(hostname)
  
  # Determine hostname based on role
  case "$ROLE" in
    primary)
      new_hostname="prd-ipa-pgdb1"
      ;;
    standby)
      new_hostname="prd-ipa-pgdb2"
      ;;
    witness)
      new_hostname="prd-ipa-pgdb-witness"
      ;;
    *)
      warn "Unknown role: $ROLE, skipping hostname configuration"
      return 0
      ;;
  esac
  ...
  info "Setting hostname from '$current_hostname' to '$new_hostname'"
  
  # Set hostname using hostnamectl (preferred method)
  if command -v hostnamectl >/dev/null 2>&1; then
    if hostnamectl set-hostname "$new_hostname"; then
      success "Hostname set using hostnamectl"
    else
      error "Failed to set hostname using hostnamectl"
      return 1
    fi
  else
    # Fallback method
    ...
    success "Hostname set using traditional method"
  fi
  ...
  if ! grep -q "127.0.1.1.*$new_hostname" /etc/hosts 2>/dev/null; then
    ...
    success "Updated /etc/hosts with new hostname"
  fi
  
  # Add entry with actual IP if available
  if [[ -n "$SELF_IP" ]]; then
    if ! grep -q "$SELF_IP.*$new_hostname" /etc/hosts 2>/dev/null; then
      echo "$SELF_IP $new_hostname" >> /etc/hosts
      info "Added entry: $SELF_IP $new_hostname"
    fi
  fi
  ...
  if [[ "$verify_hostname" == "$new_hostname" ]]; then
    success "Hostname verified: $verify_hostname"
  else
    warn "Hostname verification failed - expected: $new_hostname, got: $verify_hostname"
  fi
  
  # Update /etc/cloud/cloud.cfg to preserve hostname (for cloud instances)
  if [[ -f /etc/cloud/cloud.cfg ]]; then
    info "Configuring cloud-init to preserve hostname..."
    if ! grep -q "preserve_hostname: true" /etc/cloud/cloud.cfg 2>/dev/null; then
      ...
      info "Cloud-init already configured to preserve hostname"
    fi
  fi
  
  success "Hostname configuration completed: $new_hostname"
}
...
load_secrets() {
  ...
  local pg_appuser_secret="$(get_metadata pg_appuser_secret_id)"
  local pg_wso2user_secret="$(get_metadata pg_wso2user_secret_id)"
  local pg_tmsuser_secret="$(get_metadata pg_tmsuser_secret_id)"
  local pg_examuser_secret="$(get_metadata pg_examuser_secret_id)"
  local pg_helpdeskuser_secret="$(get_metadata pg_helpdeskuser_secret_id)"
  local pg_konguser_secret="$(get_metadata pg_konguser_secret_id)"
  ...
  info "  → AppUser Secret: $pg_appuser_secret"
  info "  → WSO2User Secret: $pg_wso2user_secret"
  info "  → TMSUser Secret: $pg_tmsuser_secret"
  info "  → ExamUser Secret: $pg_examuser_secret"
  info "  → HelpdeskUser Secret: $pg_helpdeskuser_secret"
  info "  → KongUser Secret: $pg_konguser_secret"
  ...
  # PostgreSQL AppUser Password
  info "🔐 Loading PostgreSQL application user password..."
  if [[ -n "${PG_APPUSER_PASS:-}" ]]; then
    info "  ✓ Using pre-set app user password from environment"
  else
    export PG_APPUSER_PASS=$(get_secret_enhanced "pg_appuser" "$pg_appuser_secret")
  fi
  
  # PostgreSQL WSO2 User Password
  info "🔐 Loading PostgreSQL WSO2 user password..."
  if [[ -n "${PG_WSO2USER_PASS:-}" ]]; then
    info "  ✓ Using pre-set WSO2 user password from environment"
  else
    export PG_WSO2USER_PASS=$(get_secret_enhanced "pg_wso2user" "$pg_wso2user_secret")
  fi
  
  # PostgreSQL TMS User Password
  info "🔐 Loading PostgreSQL TMS user password..."
  if [[ -n "${PG_TMSUSER_PASS:-}" ]]; then
    info "  ✓ Using pre-set TMS user password from environment"
  else
    export PG_TMSUSER_PASS=$(get_secret_enhanced "pg_tmsuser" "$pg_tmsuser_secret")
  fi
  
  # PostgreSQL Exam User Password
  info "🔐 Loading PostgreSQL Exam user password..."
  if [[ -n "${PG_EXAMUSER_PASS:-}" ]]; then
    info "  ✓ Using pre-set Exam user password from environment"
  else
    export PG_EXAMUSER_PASS=$(get_secret_enhanced "pg_examuser" "$pg_examuser_secret")
  fi
  
  # PostgreSQL Helpdesk User Password
  info "🔐 Loading PostgreSQL Helpdesk user password..."
  if [[ -n "${PG_HELPDESKUSER_PASS:-}" ]]; then
    info "  ✓ Using pre-set Helpdesk user password from environment"
  else
    export PG_HELPDESKUSER_PASS=$(get_secret_enhanced "pg_helpdeskuser" "$pg_helpdeskuser_secret")
  fi
  
  # PostgreSQL Kong User Password
  info "🔐 Loading PostgreSQL Kong user password..."
  if [[ -n "${PG_KONGUSER_PASS:-}" ]]; then
    info "  ✓ Using pre-set Kong user password from environment"
  else
    export PG_KONGUSER_PASS=$(get_secret_enhanced "pg_konguser" "$pg_konguser_secret")
  fi
  ...
  # Validate all passwords are properly loaded
  local password_validation_failed=0
  
  if [[ -z "$PG_SUPER_PASS" || "$PG_SUPER_PASS" == "changeMe" || "${#PG_SUPER_PASS}" -lt 8 ]]; then
    warn "Superuser password validation failed - generating fallback"
  ...
  if [[ -z "$PG_REPL_PASS" || "$PG_REPL_PASS" == "changeMe" || "${#PG_REPL_PASS}" -lt 8 ]]; then
    warn "Replication password validation failed - generating fallback"
  ...
  if [[ -z "$PG_MONITOR_PASS" || "$PG_MONITOR_PASS" == "changeMe" || "${#PG_MONITOR_PASS}" -lt 8 ]]; then
    warn "Monitor password validation failed - generating fallback"
    export PG_MONITOR_PASS=$(gen_pw)
    info "  → New monitor generated password length: ${#PG_MONITOR_PASS} characters"
  fi
  
  if [[ -z "$PG_APPUSER_PASS" || "$PG_APPUSER_PASS" == "changeMe" || "${#PG_APPUSER_PASS}" -lt 8 ]]; then
    warn "AppUser password validation failed - generating fallback"
    export PG_APPUSER_PASS=$(gen_pw)
    info "  → New app user generated password length: ${#PG_APPUSER_PASS} characters"
  fi
  
  if [[ -z "$PG_WSO2USER_PASS" || "$PG_WSO2USER_PASS" == "changeMe" || "${#PG_WSO2USER_PASS}" -lt 8 ]]; then
    warn "WSO2 user password validation failed - generating fallback"
    export PG_WSO2USER_PASS=$(gen_pw)
    info "  → New WSO2 user generated password length: ${#PG_WSO2USER_PASS} characters"
  fi
  
  if [[ -z "$PG_TMSUSER_PASS" || "$PG_TMSUSER_PASS" == "changeMe" || "${#PG_TMSUSER_PASS}" -lt 8 ]]; then
    warn "TMS user password validation failed - generating fallback"
    export PG_TMSUSER_PASS=$(gen_pw)
    info "  → New TMS user generated password length: ${#PG_TMSUSER_PASS} characters"
  fi
  
  if [[ -z "$PG_EXAMUSER_PASS" || "$PG_EXAMUSER_PASS" == "changeMe" || "${#PG_EXAMUSER_PASS}" -lt 8 ]]; then
    warn "Exam user password validation failed - generating fallback"
    export PG_EXAMUSER_PASS=$(gen_pw)
    info "  → New Exam user generated password length: ${#PG_EXAMUSER_PASS} characters"
  fi
  
  if [[ -z "$PG_HELPDESKUSER_PASS" || "$PG_HELPDESKUSER_PASS" == "changeMe" || "${#PG_HELPDESKUSER_PASS}" -lt 8 ]]; then
    warn "Helpdesk user password validation failed - generating fallback"
    export PG_HELPDESKUSER_PASS=$(gen_pw)
    info "  → New Helpdesk user generated password length: ${#PG_HELPDESKUSER_PASS} characters"
  fi
  
  if [[ -z "$PG_KONGUSER_PASS" || "$PG_KONGUSER_PASS" == "changeMe" || "${#PG_KONGUSER_PASS}" -lt 8 ]]; then
    warn "Kong user password validation failed - generating fallback"
    export PG_KONGUSER_PASS=$(gen_pw)
    info "  → New Kong user generated password length: ${#PG_KONGUSER_PASS} characters"
  fi
  
  if [[ -z "$PGBOUNCER_PASSWORD" || "$PGBOUNCER_PASSWORD" == "changeMe" || "${#PGBOUNCER_PASSWORD}" -lt 8 ]]; then
    warn "PgBouncer password validation failed - generating fallback"
  ...
  info "  → PostgreSQL AppUser: ${#PG_APPUSER_PASS} characters ✓"
  info "  → PostgreSQL WSO2User: ${#PG_WSO2USER_PASS} characters ✓"
  info "  → PostgreSQL TMSUser: ${#PG_TMSUSER_PASS} characters ✓"
  info "  → PostgreSQL ExamUser: ${#PG_EXAMUSER_PASS} characters ✓"
  info "  → PostgreSQL HelpdeskUser: ${#PG_HELPDESKUSER_PASS} characters ✓"
  info "  → PostgreSQL KongUser: ${#PG_KONGUSER_PASS} characters ✓"
...
install_packages() {
  ...
  retry 5 1 apt update -y
  retry 5 1 apt install -y wget ca-certificates gnupg lsb-release curl gpg vim xfsprogs lvm2
  ...
  # Set ownership on LVM mount if needed (now that postgres user exists)
  if [[ "${LVM_MOUNT_NEEDS_OWNERSHIP:-false}" == "true" ]]; then
    local mount_point="${POSTGRES_MOUNT:-/var/lib/postgresql}"
    info "Setting ownership for postgres user on $mount_point"
    chown -R postgres:postgres "$mount_point"
    chmod 750 "$mount_point"
    success "Ownership set for postgres user"
  fi
...
setup_pgpass() {
  info "Setting up .pgpass file for password-less authentication..."
...
*:5432:*:repuser:${PG_REPL_PASS}
*:5432:*:postgres:${PG_SUPER_PASS}
*:5432:*:app_user:${PG_APPUSER_PASS}
*:5432:*:wso2_user:${PG_WSO2USER_PASS}
*:5432:*:tms_user:${PG_TMSUSER_PASS}
*:5432:*:exam_user:${PG_EXAMUSER_PASS}
*:5432:*:helpdesk_user:${PG_HELPDESKUSER_PASS}
*:5432:*:kong_user:${PG_KONGUSER_PASS}
*:5432:*:monitor_user:${PG_MONITOR_PASS}
*:6432:*:postgres:${PG_SUPER_PASS}
*:6432:*:app_user:${PG_APPUSER_PASS}
*:6432:*:wso2_user:${PG_WSO2USER_PASS}
*:6432:*:tms_user:${PG_TMSUSER_PASS}
*:6432:*:exam_user:${PG_EXAMUSER_PASS}
*:6432:*:helpdesk_user:${PG_HELPDESKUSER_PASS}
*:6432:*:kong_user:${PG_KONGUSER_PASS}
...
EOF
  ...
  success ".pgpass file configured with all user credentials"
}
...
create_pgbouncer_userlist() {
  info "Creating PgBouncer userlist file with MD5 hashes..."
  
  # Generate MD5 hashes for all users
  local postgres_md5=$(md5_hash "postgres" "$PG_SUPER_PASS")
  local repuser_md5=$(md5_hash "repuser" "$PG_REPL_PASS")
  local monitor_md5=$(md5_hash "monitor_user" "$PG_MONITOR_PASS")
  local appuser_md5=$(md5_hash "app_user" "$PG_APPUSER_PASS")
  local wso2user_md5=$(md5_hash "wso2_user" "$PG_WSO2USER_PASS")
  local tmsuser_md5=$(md5_hash "tms_user" "$PG_TMSUSER_PASS")
  local examuser_md5=$(md5_hash "exam_user" "$PG_EXAMUSER_PASS")
  local helpdeskuser_md5=$(md5_hash "helpdesk_user" "$PG_HELPDESKUSER_PASS")
  local konguser_md5=$(md5_hash "kong_user" "$PG_KONGUSER_PASS")
...
;; Format: "username" "md5<md5_hash>"
"postgres" "md5${postgres_md5}"
"repuser" "md5${repuser_md5}"
"monitor_user" "md5${monitor_md5}"
"app_user" "md5${appuser_md5}"
...
EOF
  ...
  chown postgres:pgbouncer "$PGBOUNCER_USERLIST_FILE"

  success "PgBouncer userlist file created with MD5 password hashes for all users"
}
...
init_primary() {
  ...
  local repl_exists=$(sudo -u postgres psql -Atqc "SELECT COUNT(*) FROM pg_roles WHERE rolname = 'repuser';" 2>/dev/null || echo "0")
  ...
  local wso2_exists=$(sudo -u postgres psql -Atqc "SELECT COUNT(*) FROM pg_roles WHERE rolname = 'wso2_user';" 2>/dev/null || echo "0")
  local tms_exists=$(sudo -u postgres psql -Atqc "SELECT COUNT(*) FROM pg_roles WHERE rolname = 'tms_user';"  2>/dev/null || echo "0")
  local exam_exists=$(sudo -u postgres psql -Atqc "SELECT COUNT(*) FROM pg_roles WHERE rolname = 'exam_user';" 2>/dev/null || echo "0")
  local helpdesk_exists=$(sudo -u postgres psql -Atqc "SELECT COUNT(*) FROM pg_roles WHERE rolname = 'helpdesk_user';" 2>/dev/null || echo "0")
  local kong_exists=$(sudo -u postgres psql -Atqc "SELECT COUNT(*) FROM pg_roles WHERE rolname = 'kong_user';" 2>/dev/null || echo "0")
  
  info "Current users - repuser: $repl_exists, monitor_user: $monitor_exists, app_user: $app_exists"
  info "Additional users - wso2_user: $wso2_exists, tms_user: $tms_exists, exam_user: $exam_exists, helpdesk_user: $helpdesk_exists, kong_user: $kong_exists"
  ...
  if [[ "$repl_exists" == "0" ]]; then
    info "Creating replication user (repuser)..."
    if timeout 30 sudo -u postgres psql -c "CREATE USER repuser WITH REPLICATION LOGIN;" >/dev/null 2>&1; then
      success "Created replication user (repuser)"
    else
      warn "Failed to create replication user - may already exist or timed out"
    fi
  else
    info "Replication user (repuser) already exists"
  fi
  ...
  # Create wso2_user if missing (with timeout)
  if [[ "$wso2_exists" == "0" ]]; then
    if timeout 30 sudo -u postgres psql -c "CREATE USER wso2_user WITH LOGIN;" >/dev/null 2>&1; then
      success "Created wso2_user"
    else
      warn "Failed to create wso2_user - may already exist or timed out"
    fi
  else
    info "WSO2 user already exists"
  fi
  
  # Create tms_user if missing (with timeout)
  if [[ "$tms_exists" == "0" ]]; then
    if timeout 30 sudo -u postgres psql -c "CREATE USER tms_user WITH LOGIN;" >/dev/null 2>&1; then
      success "Created tms_user"
    else
      warn "Failed to create tms_user - may already exist or timed out"
    fi
  else
    info "TMS user already exists"
  fi
  
  # Create exam_user if missing (with timeout)
  if [[ "$exam_exists" == "0" ]]; then
    if timeout 30 sudo -u postgres psql -c "CREATE USER exam_user WITH LOGIN;" >/dev/null 2>&1; then
      success "Created exam_user"
    else
      warn "Failed to create exam_user - may already exist or timed out"
    fi
  else
    info "Exam user already exists"
  fi
  
  # Create helpdesk_user if missing (with timeout)
  if [[ "$helpdesk_exists" == "0" ]]; then
    if timeout 30 sudo -u postgres psql -c "CREATE USER helpdesk_user WITH LOGIN;" >/dev/null 2>&1; then
      success "Created helpdesk_user"
    else
      warn "Failed to create helpdesk_user - may already exist or timed out"
    fi
  else
    info "Helpdesk user already exists"
  fi
  
  # Create kong_user if missing (with timeout)
  if [[ "$kong_exists" == "0" ]]; then
    if timeout 30 sudo -u postgres psql -c "CREATE USER kong_user WITH LOGIN;" >/dev/null 2>&1; then
      success "Created kong_user"
    else
      warn "Failed to create kong_user - may already exist or timed out"
    fi
  else
    info "Kong user already exists"
  fi
  ...
  timeout 30 sudo -u postgres psql -c "ALTER USER repuser PASSWORD '${PG_REPL_PASS}';" || warn "Failed to set replication password"
  ...
  timeout 30 sudo -u postgres psql -c "ALTER USER app_user PASSWORD '${PG_APPUSER_PASS}';" || warn "Failed to set app password"
  timeout 30 sudo -u postgres psql -c "ALTER USER wso2_user PASSWORD '${PG_WSO2USER_PASS}';" || warn "Failed to set wso2_user password"
  timeout 30 sudo -u postgres psql -c "ALTER USER tms_user PASSWORD '${PG_TMSUSER_PASS}';" || warn "Failed to set tms_user password"
  timeout 30 sudo -u postgres psql -c "ALTER USER exam_user PASSWORD '${PG_EXAMUSER_PASS}';" || warn "Failed to set exam_user password"
  timeout 30 sudo -u postgres psql -c "ALTER USER helpdesk_user PASSWORD '${PG_HELPDESKUSER_PASS}';" || warn "Failed to set helpdesk_user password"
  timeout 30 sudo -u postgres psql -c "ALTER USER kong_user PASSWORD '${PG_KONGUSER_PASS}';" || warn "Failed to set kong_user password"
  
  # Grant permissions with enhanced monitoring user setup
...
GRANT CONNECT ON DATABASE postgres TO wso2_user;
GRANT CONNECT ON DATABASE postgres TO tms_user;
GRANT CONNECT ON DATABASE postgres TO exam_user;
GRANT CONNECT ON DATABASE postgres TO helpdesk_user;
GRANT CONNECT ON DATABASE postgres TO kong_user;
GRANT CONNECT ON DATABASE template1 TO app_user;
GRANT USAGE ON SCHEMA public TO app_user;
GRANT USAGE ON SCHEMA public TO wso2_user;
...
-- Grant pg_read_all_stats for monitoring
GRANT pg_read_all_stats TO monitor_user;
EOF
  
  # Verify users were created successfully
  info "Verifying created users:"
  sudo -u postgres psql -c "SELECT rolname, rolreplication, rolcanlogin FROM pg_roles WHERE rolname IN ('repuser', 'monitor_user', 'app_user', 'wso2_user', 'tms_user', 'exam_user', 'helpdesk_user', 'kong_user', 'postgres') ORDER BY rolname;" || true
  ...
  sudo -u postgres psql -c "SELECT rolname, substr(rolpassword, 1, 5) as password_type FROM pg_authid WHERE rolname IN ('postgres', 'repuser') ORDER BY rolname;" || true
  
  success "Database users created with proper permissions for streaming replication"
  
  # Keep synchronous replication disabled during bootstrap (enable after standby is set up)
  info "Keeping synchronous replication disabled during bootstrap..."
  sudo -u postgres psql <<EOF || warn "Failed to configure replication settings"
-- Keep synchronous replication disabled until standby is ready
ALTER SYSTEM SET synchronous_standby_names = '';
ALTER SYSTEM SET synchronous_commit = 'local';
SELECT pg_reload_conf();
EOF
  
  info "Waiting for standby to connect before enabling synchronous replication..."
  info "Note: This is optional during bootstrap - synchronous replication can be enabled later"
  info "Timeout: 60 seconds (reduced from 120s for faster bootstrap)"

  local standby_wait_timeout=120
  local standby_wait_interval=5
  local elapsed_time=0
  local standby_connected=false
  
  while [[ $elapsed_time -lt $standby_wait_timeout ]]; do
    # Check if standby is connected
    local standby_count
    standby_count=$(sudo -u postgres psql -Atqc "SELECT COUNT(*) FROM pg_stat_replication WHERE application_name = 'standby_sync' AND state = 'streaming';" 2>/dev/null || echo "0")
    
    if [[ "$standby_count" -gt 0 ]]; then
      standby_connected=true
      info "Standby connected! Enabling synchronous replication..."
      
      # Enable synchronous replication
      sudo -u postgres psql <<SYNC_EOF || warn "Failed to enable synchronous replication"
-- Enable synchronous replication now that standby is connected
ALTER SYSTEM SET synchronous_standby_names = 'FIRST 1 (standby_sync)';
ALTER SYSTEM SET synchronous_commit = 'on';
SELECT pg_reload_conf();
SYNC_EOF
      
      success "✅ Synchronous replication enabled successfully!"
      ...
      if [[ "$sync_state" == "sync" ]]; then
        ...
        info "ℹ️ Standby sync state: $sync_state (will become 'sync' after standby fully initializes)"
      fi
      
      break
    fi
    
    # Show progress every 30 seconds (but not on first iteration)
    if [[ $elapsed_time -gt 0 && $((elapsed_time % 30)) -eq 0 ]]; then
      info "Waiting for standby connection... ($elapsed_time/$standby_wait_timeout seconds elapsed)"
    fi
    
    sleep $standby_wait_interval
    elapsed_time=$((elapsed_time + standby_wait_interval))
  done
  
  if [[ "$standby_connected" == false ]]; then
    warn "⚠️ Standby did not connect within $standby_wait_timeout seconds"
    warn "⚠️ Synchronous replication remains DISABLED"
    warn ""
    warn "📋 Enable synchronous replication manually after standby is configured:"
    warn "  sudo -u postgres psql <<EOF"
    warn "  ALTER SYSTEM SET synchronous_standby_names = 'FIRST 1 (standby_sync)';"
    warn "  ALTER SYSTEM SET synchronous_commit = 'on';"
    warn "  SELECT pg_reload_conf();"
    warn "  EOF"
    warn ""
    warn "📋 Check replication status:"
    warn "  sudo -u postgres psql -c \"SELECT application_name, state, sync_state FROM pg_stat_replication;\""
  fi
  ...
  local slot_exists=$(sudo -u postgres psql -Atqc "SELECT COUNT(*) FROM pg_replication_slots WHERE slot_name = 'pgstandby1';" 2>/dev/null || echo "0")
  
  if [[ "$slot_exists" == "0" ]]; then
    if sudo -u postgres psql -c "SELECT pg_create_physical_replication_slot('pgstandby1');" >/dev/null 2>&1; then
      success "Created replication slot 'pgstandby1'"
      info "Note: Replication slot will become active once standby connects"
    else
      warn "Failed to create replication slot - may already exist"
    fi
  else
    info "Replication slot 'pgstandby1' already exists"
  fi
  ...
  # Create sentinel directory and file (ensure it exists after LVM mount)
  mkdir -p "$SENTINEL_DIR"
  chown postgres:postgres "$SENTINEL_DIR" 2>/dev/null || true
  chmod 755 "$SENTINEL_DIR"
  touch "$SENTINEL_PRIMARY_INIT"
  chown postgres:postgres "$SENTINEL_PRIMARY_INIT" 2>/dev/null || true
...
init_standby() {
  ...
  if ! sudo -u postgres env PGPASSFILE=/var/lib/postgresql/.pgpass \
       pg_basebackup -h "$PRIMARY_HOST" -D "$PG_DATA_DIR" -U repuser \
       -v -P -W -R; then
    ...
    sudo -u postgres env PGPASSFILE=/var/lib/postgresql/.pgpass \
         psql -h "$PRIMARY_HOST" -U repuser -c "SELECT version();" || true
    return 1
  fi
  ...
  # Note: Not using primary_slot_name to allow immediate connection without slot dependency
  sudo -u postgres cat >> "$PG_DATA_DIR/postgresql.auto.conf" <<EOF
# Standby configuration for streaming replication
primary_conninfo = 'host=$PRIMARY_HOST port=5432 user=repuser application_name=standby_sync'
restore_command = 'cp /var/lib/postgresql/wal_archive/%f %p'
recovery_target_timeline = 'latest'
EOF
  ...
  if [[ "$recovery_status" == "t" ]]; then
    ...
    # Check replication connection with retry (may take a moment to establish)
    info "Checking replication connection..."
    local repl_status=""
    local retry_count=0
    while [[ $retry_count -lt 10 ]]; do
      repl_status=$(sudo -u postgres psql -Atqc "SELECT status FROM pg_stat_wal_receiver;" 2>/dev/null || echo "")
    ...
    if [[ -z "$wal_receiver" || "$wal_receiver" == "" ]]; then
      info "WAL receiver starting up (may take a few more seconds)"
    fi
  else
  ...
  chown postgres:postgres "$SENTINEL_STANDBY_CLONED" 2>/dev/null || true
  
  success "Standby node initialized with streaming replication"
}

sync_database_passwords() {
  info "Synchronizing database passwords with PgBouncer (MD5 compatibility)"
  ...
  if [[ "$is_standby" == "t" ]]; then
    info "Standby node detected - all users should already exist from replication"
    info "Passwords will be replicated from primary - no changes needed on read-only standby"
  else
    info "Primary node - ensuring all users have MD5 passwords for PgBouncer compatibility"
    ...
    # Ensure MD5 password encryption for PgBouncer compatibility
    info "Converting all user passwords to MD5 for PgBouncer (this is required for PgBouncer auth)..."
    if sudo -u postgres psql <<EOF 2>/dev/null
-- Set password encryption to MD5 temporarily for PgBouncer compatibility
SET password_encryption = 'md5';

-- Update all user passwords with MD5 encryption
ALTER USER postgres PASSWORD '${PG_SUPER_PASS}';
ALTER USER app_user PASSWORD '${PG_APPUSER_PASS}';
ALTER USER wso2_user PASSWORD '${PG_WSO2USER_PASS}';
ALTER USER tms_user PASSWORD '${PG_TMSUSER_PASS}';
ALTER USER exam_user PASSWORD '${PG_EXAMUSER_PASS}';
ALTER USER helpdesk_user PASSWORD '${PG_HELPDESKUSER_PASS}';
ALTER USER kong_user PASSWORD '${PG_KONGUSER_PASS}';
ALTER USER monitor_user PASSWORD '${PG_MONITOR_PASS}';
ALTER USER pgbouncer_admin PASSWORD '${PGBOUNCER_PASSWORD}';

-- Restore password encryption to scram-sha-256 for new users
SET password_encryption = 'scram-sha-256';
EOF
    then
      success "All user passwords converted to MD5 for PgBouncer"
    else
      info "Password conversion completed (some users may have already had MD5 passwords)"
    fi
...
-- Ensure all application users have proper database access
GRANT CONNECT ON DATABASE postgres TO app_user;
GRANT CONNECT ON DATABASE postgres TO wso2_user;
GRANT CONNECT ON DATABASE postgres TO tms_user;
GRANT CONNECT ON DATABASE postgres TO exam_user;
...
EOF
    ...
    # Regenerate PgBouncer userlist with fresh MD5 hashes
    info "Regenerating PgBouncer userlist with updated MD5 hashes..."
    create_pgbouncer_userlist
  fi
  
  success "Database passwords synchronized with PgBouncer"
}
...
setup_pg_extensions() {
  info "Setting up PostgreSQL extensions (pg_stat_statements, pg_buffercache, uuid-ossp, plpgsql)..."
  
  # Wait for PostgreSQL to be ready
  if ! retry 30 2 sudo -u postgres psql -c "SELECT 1" >&/dev/null; then
    warn "PostgreSQL not ready, skipping extensions setup"
    return 1
  fi
  ...
  if [[ "$is_standby" == "t" ]]; then
    info "Standby node detected - extensions should already exist from replication"
    info "Verifying extensions on standby (read-only verification)..."
    
    # Wait a moment for replication to catch up after base backup
    sleep 2
    
    # Verify all 4 extensions exist on standby with retry logic
    local ext_count=0
    local retry_count=0
    local max_retries=5
    
    while [[ $retry_count -lt $max_retries && $ext_count -lt 4 ]]; do
      ext_count=$(sudo -u postgres psql -d postgres -Atqc "SELECT COUNT(*) FROM pg_extension WHERE extname IN ('pg_stat_statements', 'pg_buffercache', 'uuid-ossp', 'plpgsql');" 2>/dev/null || echo "0")
      
      if [[ "$ext_count" -gt 0 ]]; then
        success "All 4 extensions verified on standby node"
        sudo -u postgres psql -d postgres -c "SELECT extname, extversion, nspname as schema FROM pg_extension e JOIN pg_namespace n ON e.extnamespace = n.oid WHERE extname IN ('pg_stat_statements', 'pg_buffercache', 'uuid-ossp', 'plpgsql') ORDER BY extname;" 2>/dev/null || true
        break
      else
        retry_count=$((retry_count + 1))
        if [[ $retry_count -lt $max_retries ]]; then
          info "Extension replication in progress... ($ext_count/4 extensions found, retry $retry_count/$max_retries)"
          sleep 2
        fi
      fi
    done
    
    if [[ "$ext_count" -lt 4 ]]; then
      warn "Expected 4 extensions on standby, found: $ext_count (may still be replicating)"
      info "Extensions will be automatically available once replication completes"
    fi
    
    return 0
  fi
  
  # Create extensions on primary node (in both postgres and template1 databases)
  info "Creating PostgreSQL extensions on primary node..."
  
  # Install extensions in postgres database
  info "Installing extensions in 'postgres' database..."
  sudo -u postgres psql -d postgres <<'EXT_POSTGRES_EOF' || warn "Some extensions may have failed to create in postgres database"
-- Ensure plpgsql is enabled (usually installed by default but verify)
CREATE EXTENSION IF NOT EXISTS plpgsql;

-- Create pg_stat_statements extension for query performance monitoring
CREATE EXTENSION IF NOT EXISTS pg_stat_statements;

-- Create pg_buffercache extension for buffer cache inspection
...
EXT_POSTGRES_EOF
  ...
  # Install extensions in template1 database (for future databases)
  info "Installing extensions in 'template1' database..."
  sudo -u postgres psql -d template1 <<'EXT_TEMPLATE_EOF' || warn "Some extensions may have failed to create in template1 database"
-- Ensure plpgsql is enabled
CREATE EXTENSION IF NOT EXISTS plpgsql;

-- Create pg_stat_statements extension
CREATE EXTENSION IF NOT EXISTS pg_stat_statements;

-- Create pg_buffercache extension
CREATE EXTENSION IF NOT EXISTS pg_buffercache;
...
EXT_TEMPLATE_EOF
  ...
  # Verify extensions were created in postgres database
  info "Verifying installed extensions in 'postgres' database:"
  local ext_verification
  ext_verification=$(sudo -u postgres psql -d postgres -Atqc "SELECT COUNT(*) FROM pg_extension WHERE extname IN ('plpgsql', 'pg_stat_statements', 'pg_buffercache', 'uuid-ossp');" 2>/dev/null || echo "0")
  
  if [[ "$ext_verification" == "4" ]]; then
    success "✅ All 4 required extensions installed successfully in 'postgres' database"
  else
    warn "⚠️ Expected 4 extensions, found: $ext_verification"
  fi
  
  # Display detailed extension information
  info "Extension details:"
  sudo -u postgres psql -d postgres <<'EXT_INFO_EOF'
SELECT 
  ...
  CASE 
    ...
    WHEN e.extname = 'pg_stat_statements' THEN 'Track planning and execution statistics of all SQL statements'
...
EXT_INFO_EOF
  ...
  info "Verifying extensions in 'template1' database:"
  local template1_ext_count
  template1_ext_count=$(sudo -u postgres psql -d template1 -Atqc "SELECT COUNT(*) FROM pg_extension WHERE extname IN ('plpgsql', 'pg_stat_statements', 'pg_buffercache', 'uuid-ossp');" 2>/dev/null || echo "0")
  
  if [[ "$template1_ext_count" == "4" ]]; then
    success "✅ All 4 extensions also installed in 'template1' database (for future databases)"
  else
    info "Template1 extensions: $template1_ext_count (this is optional)"
  fi
  ...
  if sudo -u postgres psql -d postgres -Atqc "SELECT COUNT(*) FROM pg_stat_statements;" >/dev/null 2>&1; then
    success "✅ pg_stat_statements is functional"
  else
    warn "⚠️ pg_stat_statements query test failed"
  fi
  
  # Test pg_buffercache
  if sudo -u postgres psql -d postgres -Atqc "SELECT COUNT(*) FROM pg_buffercache LIMIT 1;" >/dev/null 2>&1; then
    success "✅ pg_buffercache is functional"
  else
    warn "⚠️ pg_buffercache query test failed"
  fi
  
  # Test uuid-ossp
  local test_uuid
  test_uuid=$(sudo -u postgres psql -d postgres -Atqc "SELECT uuid_generate_v4();" 2>/dev/null || echo "")
  if [[ -n "$test_uuid" && "$test_uuid" =~ ^[0-9a-f]{8}-[0-9a-f]{4}-[4][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$ ]]; then
    success "✅ uuid-ossp is functional (generated: $test_uuid)"
  else
    warn "⚠️ uuid-ossp functionality test inconclusive"
  fi
  
  # Test plpgsql
  if sudo -u postgres psql -d postgres -Atqc "SELECT lanname FROM pg_language WHERE lanname = 'plpgsql';" 2>/dev/null | grep -q "plpgsql"; then
    success "✅ plpgsql language is available"
  else
    warn "⚠️ plpgsql language not found"
  fi
  
  success "PostgreSQL extensions (plpgsql, pg_stat_statements, pg_buffercache, uuid-ossp) installation complete"
}
    ...
    if systemctl start pgbouncer.service; then
      ...
      if timeout 10 sudo -u postgres env PGPASSFILE=/var/lib/postgresql/.pgpass \
        ...
        if [[ "$pg_pass_type" == "SCRAM" ]]; then
          info "PostgreSQL postgres user still has SCRAM password, converting to MD5..."
          
          # Use a more robust approach to convert password
          if sudo -u postgres psql <<PG_MD5_EOF >/dev/null 2>&1
-- Set password encryption to MD5 temporarily for PgBouncer compatibility
SET password_encryption = 'md5';
...
PG_MD5_EOF
          then
            success "Converted all user passwords to MD5 for PgBouncer compatibility"
            
            # Regenerate PgBouncer userlist with all correct MD5 hashes
            info "Regenerating complete PgBouncer userlist with MD5 hashes..."
            create_pgbouncer_userlist
          else
            warn "Failed to convert passwords - trying alternative method"
            
            # Alternative: Update passwords one by one
            timeout 30 sudo -u postgres psql -c "SET password_encryption = 'md5'; ALTER USER postgres PASSWORD '${PG_SUPER_PASS}';" 2>/dev/null || info "postgres password already set or failed"
            timeout 30 sudo -u postgres psql -c "SET password_encryption = 'md5'; ALTER USER app_user PASSWORD '${PG_APPUSER_PASS}';" 2>/dev/null || info "app_user password already set or failed"
            timeout 30 sudo -u postgres psql -c "SET password_encryption = 'md5'; ALTER USER monitor_user PASSWORD '${PG_MONITOR_PASS}';" 2>/dev/null || info "monitor_user password already set or failed"
        ...
        info "Restarting PgBouncer with updated configuration..."
        systemctl restart pgbouncer
        sleep 3
        
        # Test all users
        if timeout 10 sudo -u postgres env PGPASSFILE=/var/lib/postgresql/.pgpass \
           psql -h localhost -p 6432 -U postgres -d postgres -c "SELECT 'PgBouncer postgres authentication test';" >/dev/null 2>&1; then
          success "PgBouncer postgres authentication recovered after MD5 conversion"
        else
          warn "PgBouncer postgres authentication still failing"
        fi
        
        # Test monitor_user specifically - also convert if needed (skip on standby)
        local is_standby_check
        is_standby_check=$(sudo -u postgres psql -Atqc "SELECT pg_is_in_recovery();" 2>/dev/null || echo "unknown")
        
        if [[ "$is_standby_check" != "t" ]]; then
          info "Ensuring monitor_user has MD5 password..."
          timeout 30 sudo -u postgres psql -c "SET password_encryption = 'md5'; ALTER USER monitor_user PASSWORD '${PG_MONITOR_PASS}';" 2>/dev/null || info "monitor_user password update skipped or already set"
        else
          info "Standby node - skipping monitor_user password update (read-only)"
        fi
        
        # Regenerate userlist again with monitor_user
        create_pgbouncer_userlist
        systemctl restart pgbouncer
        sleep 2
        
        if timeout 10 sudo -u postgres env PGPASSFILE=/var/lib/postgresql/.pgpass \
           psql -h localhost -p 6432 -U monitor_user -d postgres -c "SELECT 1;" >/dev/null 2>&1; then
          success "PgBouncer monitor_user authentication working"
        else
          info "PgBouncer monitor_user still has issues - verify password in userlist"
        fi
      fi
    else
  ...
  if [[ "$ROLE" == "standby" ]]; then
    ...
    if systemctl start pg-failover-manager.service; then
      ...
      # Verify replication is working with retry logic
      sleep 3
      ...
      if [[ "$replication_status" == "t" ]]; then
        ...
        # Check WAL receiver with retry (may take a moment to start)
        local wal_receiver=""
        local retry_count=0
        while [[ $retry_count -lt 10 ]]; do
...
main() {
  ...
  detect_configuration
  set_timezone
  set_hostname  # Set hostname based on role (primary/standby/witness)
  setup_lvm_storage  # Setup LVM storage before PostgreSQL installation
  load_secrets
  install_packages
  setup_pgpass
  # configure_postgresql
