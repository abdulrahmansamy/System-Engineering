...
push_backup() {
  local src="$1"
  local type="$2"
  local retention="$3"   # daily, weekly, monthly
  local target="pgsql/${retention}"

  log_info "Uploading ${retention} ${type} backup from ${src} to ${target}"
  gsutil -m cp -r "${src}"/* "${BUCKET}/${target}/${type}/${DATE}/" >>"$LOGFILE" 2>&1
  if [ $? -eq 0 ]; then
    log_success "${retention} ${type} backup pushed to ${target}"
  else
    log_fail "${retention} ${type} backup push to ${target}"
  fi
}
...
case "$1" in
  logical)
    ...
    push_backup "${BASE}/AllDBData" "logical" "daily"
    ;;
  physical)
    # Daily physical backup
    push_backup "${BASE}/phybackup" "physical" "daily"
    ;;
  wal)
    # WAL backups every 6 hours
    push_backup "${BASE}/walbackup" "wal" "daily"
    ;;
  weekly)
    # Copy logical backup into weekly retention
    push_backup "${BASE}/AllDBData" "logical" "weekly"
    # Copy physical backup into weekly retention
    push_backup "${BASE}/phybackup" "physical" "weekly"
    ;;
  monthly)
    # Copy logical backup into monthly retention
    push_backup "${BASE}/AllDBData" "logical" "monthly"
    # Copy physical backup into monthly retention
    push_backup "${BASE}/phybackup" "physical" "monthly"
    ;;
  *)
