...
locals {
  ip_names = {
    witness = format("%s-%s-%s-%s-%02d", local.org_code, local.env_code, "ip", "pg-witness", 1)
...
resource "google_compute_address" "witness_ip" {
  provider     = google.db_projects
  name         = local.ip_names.witness
  address      = local.pg_ips["witness"]
  region       = var.region
  address_type = "INTERNAL"
  subnetwork   = local.is_production ? data.google_compute_subnetwork.prod_db.self_link : data.google_compute_subnetwork.nonprod_db.self_link
  purpose      = "GCE_ENDPOINT"
...
output "witness_internal_ip" { value = google_compute_address.witness_ip.address }
