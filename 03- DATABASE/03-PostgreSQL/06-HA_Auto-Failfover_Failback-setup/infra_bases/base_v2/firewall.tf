...
resource "google_compute_firewall" "allow_ssh" {
  ...
  source_ranges = local.is_production ? var.ssh_source_ranges : ["0.0.0.0/0", "192.168.14.0/23"]
...
resource "google_compute_firewall" "allow_pg" {
  ...
  source_ranges = local.is_production ? [var.prod_db_subnet_cidr] : [var.nonprod_db_subnet_cidr]
...
resource "google_compute_firewall" "allow_icmp_internal" {
  ...
  allow { protocol = "icmp" }
  source_ranges = local.is_production ? [var.prod_app_subnet_cidr, var.prod_db_subnet_cidr] : [var.nonprod_app_subnet_cidr, var.nonprod_db_subnet_cidr]
