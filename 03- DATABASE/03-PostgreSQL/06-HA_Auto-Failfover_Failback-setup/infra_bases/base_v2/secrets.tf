...
locals {
  secret_ids = {
    ...
    tls_app_db_ilb_cert = format("%s-%s-%s-%s-%02d", local.org_code, local.env_code, "sec", "tls-app-db-ilb-crt", 1)
    tls_pg_servers_cert = format("%s-%s-%s-%s-%02d", local.org_code, local.env_code, "sec", "tls-pg-servers-crt", 1)
  }
}
...
resource "google_secret_manager_secret" "tls_pg_servers_cert" {
  count     = var.create_secrets ? 1 : 0
  project   = local.is_production ? var.prod_db_project_id : var.nonprod_db_project_id
  secret_id = local.secret_ids.tls_pg_servers_cert
  replication {
    auto {}
  }
...
resource "google_secret_manager_secret" "tls_app_db_ilb_cert" {
  count     = var.create_secrets ? 1 : 0
  project   = local.is_production ? var.prod_db_project_id : var.nonprod_db_project_id
  secret_id = local.secret_ids.tls_app_db_ilb_cert
  replication {
    auto {}
  }
...
resource "google_secret_manager_secret_version" "tls_pg_servers_crt" {
  count       = var.create_secrets ? 1 : 0
  secret      = google_secret_manager_secret.tls_pg_servers_cert[0].id
  secret_data = tls_locally_signed_cert.pg_servers.cert_pem
}

resource "google_secret_manager_secret_version" "tls_app_db_ilb_crt" {
  count       = var.create_secrets ? 1 : 0
  secret      = google_secret_manager_secret.tls_app_db_ilb_cert[0].id
  secret_data = tls_locally_signed_cert.app_db_ilb.cert_pem
}
...
resource "google_secret_manager_secret_iam_member" "tls_pg_servers_crt_accessor" {
  count     = var.create_secrets ? 1 : 0
  project   = local.is_production ? var.prod_db_project_id : var.nonprod_db_project_id
  secret_id = google_secret_manager_secret.tls_pg_servers_cert[0].id
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${google_service_account.pg_sa.email}"
}

resource "google_secret_manager_secret_iam_member" "tls_app_db_ilb_crt_accessor" {
  count     = var.create_secrets ? 1 : 0
  project   = local.is_production ? var.prod_db_project_id : var.nonprod_db_project_id
  secret_id = google_secret_manager_secret.tls_app_db_ilb_cert[0].id
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${google_service_account.pg_sa.email}"
}
